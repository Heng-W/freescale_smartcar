...
float GRAVITY_OFFSET = -530;                                               
float GRAVITY_ANGLE_MAX = 3550;                                        

float GYROSCOPE_OFFSET = -4  ;                                            
float GYROSCOPE_ANGLE_RATIO = 16.40;                               

float Car_angle_P = 0.27;                
...
float CAR_SPEED_SET = 2;                   
float SPEED_CONTROL_P = 0.25;          
...
void Angle_Calculate(void)
{
    ...
    integralDriftError = (g_fGravityAngle - g_fCarAngle) / 2.0;
    
    g_fGyroscopeAngleIntegral += (g_fGyroscopeAngleSpeed + integralDriftError) * 0.005;  
}
...
{
    float fValue;
    fValue = (44 - g_fCarAngle) * Car_angle_P + (0 - g_fGyroscopeAngleSpeed) * Car_angle_D;
...
void Speed_Control(void)
{
    float fDelta;
    float fP, fI;
    g_fCarSpeed = (leftMotorPulseSigma + rightMotorPulseSigma) * 0.5;    
    leftMotorPulseSigma = rightMotorPulseSigma = 0;
    g_fCarSpeed *= (1000.0/100/512);    
...
void Direction_Control(void)
{
    ...
    nRight = (int)(g_fRightVoltageSigma /= 2);  
...
void DirectionControlOutput(void)
{
    ...
    g_fDirectionControlOut = fValue * (g_nDirectionControlPeriod + 1) / 10 + g_fDirectionControlOutOld;
}
